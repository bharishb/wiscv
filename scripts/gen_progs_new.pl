...
sub gen_load() {
  ...
  $imm = $imm*2;
  my @insts;
  push @insts, "@load_inst[int(rand(1))] x$reg_num_dest, $imm(x$reg_num_src)";    # Vis : replacing ld with one among {lw, lh, lb, lbu, lhu}
  return @insts;
}
...
sub gen_mem_test1()     {
   ...
   for ($j = 0; $j < 5 + rand(10); $j++) {
       push @insts, "@load_inst[int(rand(1))] x$reg_num_2, 2(x$reg_num_1)";  # Vis : replacing ld with one among {lw, lh, lb, lbu, lhu}
...
sub gen_mem_test2()     {
   ...
   my $reg_num_2 = 2+int(rand(2));
   my $j = 0;
   push @insts, "addi x$reg_num_2, x$reg_num_2, 0x08";
   for ($j = 0; $j < 5 + rand(10); $j++) {
       push @insts, "sw x$reg_num_2, 4(x$reg_num_1)";  # Vis : replacing st with one among {sb, sw, sh}
       push @insts, "addi x$reg_num_1, x$reg_num_1, 4";
   }
   return @insts;
}
...
sub gen_mem_test3()     {
   ...
   push @insts, "addi x$reg_num_2, x$reg_num_2, 0x02";  # Vis : replacing lbi with addi
   ...
   for ($j = 0; $j < 5 + rand(10); $j++) {
      push @insts, "sw x$reg_num_2, 4(x$reg_num_1)";
      push @insts, "sw x$reg_num_3, 4(x$reg_num_1)";
      push @insts, "sw x$reg_num_4, 4(x$reg_num_1)"; 
   }
   return @insts;
}
...
sub gen_mem_test4()     {
   ...
   for ($index=0;$index < 255;$index++) {
       for ($offset=0;$offset<8;$offset=$offset+2) {
           ...
           push  @insts, "@load_inst[int(rand(1))] x$reg_num_2, 0(x$reg_num_1)";  # Vis : replacing ld with one among {lw, lh, lb, lbu, lhu}
       }
   }
   return @insts;
}
...
sub gen_mem_test8()      {
    ...
    push @insts, "@load_inst[int(rand(1))] x$reg_num_3, 0(x$reg_num_1)";  # Vis : replacing ld with one among {lw, lh, lb, lbu, lhu}
    return @insts;
}
